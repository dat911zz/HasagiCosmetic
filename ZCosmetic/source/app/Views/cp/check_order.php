...
<div class="container" style="min-height: 500px">
    ...
    $orders = $db->executeReader('SELECT `tbl_hoadon`.*, `tbl_nguoidung`.`HoVaTen` FROM `tbl_hoadon`, `tbl_nguoidung` WHERE `tbl_nguoidung`.`Ma` = `tbl_hoadon`.`MaNguoiDung` and `tbl_hoadon`.`MaNhanVien` is null');
    if (count($orders) == 0) { ?>
        <h5 style="color: orangered; text-align: center">Không có đơn hàng</h5>
    <?php } else { ?>
        <div class="table">
